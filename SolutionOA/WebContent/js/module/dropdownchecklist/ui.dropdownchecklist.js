    ...
    $.widget("ui.dropdownchecklist", {
        ...
        _appendItems: function() {
            ...
            if(options.firstItemChecksAll){
            	allSelected = (sourceSelect.find('option').length - 1) == sourceSelect.find('option:selected').length ? true : false;
            }else{
            	allSelected = sourceSelect.find('option').length == sourceSelect.find('option:selected').length ? true : false;
            }
			...
			sourceSelect.children().each(function(index) { // when the select has groups
                ...
                if (opt.is("option")) {
                    ...
                    if(index == 0 && options.firstItemChecksAll && allSelected){
                    	self._appendOption(opt, true, dropContainerDiv, index, false);
                    }else{
                    	self._appendOption(opt, opt.attr("selected"), dropContainerDiv, index, false);
                    }
                } else {
		...
		_appendOptions: function(parent, container, parentIndex, indent) {
			...
			parent.children("option").each(function(index) {
                ...
                if(index == 0 && options.firstItemChecksAll && allSelected){
                	self._appendOption(option, true, container, childIndex, indent);
                }else{
                	self._appendOption(option, option.attr("selected"), container, childIndex, indent);
                }
            })
		},
        _appendOption: function(option, selected, container, index, indent) {
            var self = this;
        ...
        _syncSelected: function(senderCheckbox) {
        	var sign = senderCheckbox.attr("checked") === undefined ? false : true;
            var self = this, options = this.options, sourceSelect = this.sourceSelect, controlWrapper = this.controlWrapper, dropWrapper = this.dropWrapper;
            var allCheckboxes = dropWrapper.find("input");
            if (options.firstItemChecksAll) {
            	//alert(senderCheckbox.attr("index") + '\t' + senderCheckbox.attr("checked"));
                // if firstItemChecksAll is true, check all checkboxes if the first one is checked
                if (senderCheckbox.attr("index") == 0) {
                    allCheckboxes.attr("checked", sign);
                    if(sign){
                    	$(sourceSelect.get(0).options[0]).attr('selected', false);
                    }
                } else {
            ...
            allCheckboxes.each(function(index) {
            	var s = $(this).attr("checked") === undefined ? false : true;
                $(sourceSelect.get(0).options[index]).attr('selected', s);
                if(options.firstItemChecksAll){
                	$(sourceSelect.get(0).options[0]).attr('selected', false);
                }
            });
        ...
        _setSize: function(dropCalculatedSize) {
            ...
            var maxDropHeight = (options.maxDropHeight != null) ? parseInt(options.maxDropHeight) : -1;
            var dropHeight = ((maxDropHeight > 0) && (dropCalculatedSize.height > maxDropHeight)) ? maxDropHeight : dropCalculatedSize.height;
